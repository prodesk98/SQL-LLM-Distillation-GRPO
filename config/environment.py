...
MAX_SEQ_LENGTH: int = int(config.get("max_seq_length", 1024))
GPU_MEMORY_UTILIZATION: float = float(config.get("gpu_memory_utilization", .7))
LORA_RANK: int = int(config.get("lora_rank", 16))
LORA_ALPHA: int = int(config.get("lora_alpha", 16))
MAX_PROMPT_LENGTH: int = int(config.get("max_prompt_length", 512))
...
NUM_GENERATIONS: int = int(config.get("num_generations", 6))
MAX_COMPLETION_LENGTH: int = int(config.get("max_completion_length", 1024))
